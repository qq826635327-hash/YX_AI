...
| size                | string         | 否    | 分辨率档位，支持 `720p` / `1080p`，需搭配 ratio 使用 |
| ratio               | string         | 否    | 画面比例，如 `16:9` / `9:16` / `1:1` / `4:3` / `3:4`，搭配 size 档位使用 |
...
> **分辨率指定方式**：推荐使用 `size` + `ratio` 档位写法（如 `size: "1080p", ratio: "16:9"`），也可直接传 `width` + `height` 数值。两种方式二选一，不要同时使用。视频 API 不支持图片的 `"2K"`/`"4K"` 档位，最大宽度为 1920px。
