---
alwaysApply: false
description: AI Drama Studio 项目级执行规则、工作流、编码规范与自测协议
version: 5.0
source: D:\影序AI\AGENTS.md
updated: 2026-06-21
---

> **本文件是 `D:\影序AI\AGENTS.md` 的只读镜像，请勿直接修改本文档。**
> 如需调整规则，请修改 `AGENTS.md`，然后同步覆盖本文档。
> 当前版本：v5.0 | 2026-06-21
...
| 文档 | 路径 | 用途 | AI 是否直接写入 |
|------|------|------|----------------|
| **项目规则** | `.trae/rules/project_rules.md` | AI 执行规则、工作流、验证标准 | 仅当用户要求优化规则时修改 |
...
接收需求
  ...
  → 制定计划（>3 步用 TodoWrite 拆分；>5 步或跨模块先向用户确认方案）
...
3. **新增测试时，更新 `doc/测试手册/` 对应模块文件**（如角色测试 → `03-角色.md`，单元测试 → `12-单元测试.md`）
...
为减少误操作和提高可审计性，优先使用专用工具完成对应操作：

| 操作 | 优先工具 | 避免使用的 shell 命令 |
|------|----------|----------------------|
| 读文件 | `Read` | `cat`、`head`、`tail` |
| 写/改文件 | `Write` / `Edit` | `echo`、`sed`、`awk`、重定向 |
| 搜索文件 | `Glob` / `Grep` / `SearchCodebase` | `find`、`grep`、`rg` |
| 运行命令 | `RunCommand` | — |
| 代码编辑 | `Edit` / `Write` | 任何文本替换命令 |

**原则**：能用 Trae 专用工具完成的事，就不要交给 PowerShell。
...
| 版本 | 日期 | 修订说明 |
|------|------|----------|
| v5.0 | 2026-06-21 | 明确为 `AGENTS.md` 的只读镜像；修正任务队列为 Huey；统一步骤拆分/确认阈值；细化测试手册更新指引；新增工具选择优先级与修订记录。 |
